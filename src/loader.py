...
# --- IO small wrappers using src.io  / wrappers que usan src.io 
try:
...
# CKAN loaders / cargadores CKAN

def load_gei(save=False):
    # Inventario GEI / Greenhouse Gas Inventory
    df = fetch_ckan_csv("atm_inventario_gei")  # fetch from CKAN / obtener de CKAN
    return save_clean(df, "df_gei.csv") if save else df

def load_ceee(save=False):
    # Certificados de eficiencia energética / Energy Efficiency Certificates
    df = fetch_ckan_csv("registro_certificados_eficiencia_energetica")  # fetch from CKAN / obtener de CKAN
    return save_clean(df, "df_ceee.csv") if save else df

def load_pst(save=False):
    # Planes de Sostenibilidad Turística / Tourism Sustainability Plans
    df = fetch_ckan_csv("1911600")  # fetch from CKAN / obtener de CKAN
    return save_clean(df, "df_pst.csv") if save else df


def load_heating_specs(save=False):
    # heating tech specs / especificaciones tecnológicas 
    path = ROOT / "data" / "raw" / "heating_technologies.csv"
    df = _light(_std(pd.read_csv(path)))
    if save:
        _save_df(df, str(PROC.joinpath("heating_technologies.csv")))
    return df

def load_avg_income(save=False):
    # Renta media por código postal / Average Income by postal code 
    path = ROOT / "data" / "raw" / "renta_media_madrid.csv"
    df = _light(_std(pd.read_csv(path)))
    if save:
        _save_df(df, str(PROC.joinpath("renta_media_madrid.csv")))
    return df

def load_stakeholder_map(save=False):  
    # Mapa Actores / Stakeholders Map 
    path = ROOT / "data" / "raw" / "stakeholder_map.csv"
    df = _light(_std(pd.read_csv(path)))
    if save:
        _save_df(df, str(PROC.joinpath("stakeholder_map.csv")))
    return df
